...
public class PushSenderJPushImpl implements PushSender {
    ...
    private void pushToTagOr(PushMessage message,
                             PushResponseReceiver responseReceiver) {
        PushPayload payload = createPayloadBuilder(message).setAudience(Audience.tag(message.getTags()))
                                                           .build();
        doPush(message, payload, responseReceiver);
    }
    
    private void pushToTagAnd(PushMessage message,
                              PushResponseReceiver responseReceiver) {
        PushPayload payload = createPayloadBuilder(message).setAudience(Audience.tag_and(message.getTags()))
                                                           .build();
        doPush(message, payload, responseReceiver);
    }
    
    private void pushToAlias(PushMessage message,
                             PushResponseReceiver responseReceiver) {
        PushPayload payload = createPayloadBuilder(message).setAudience(Audience.alias(message.getAlias()))
                                                           .build();
        doPush(message, payload, responseReceiver);
    }
    ...
    private void processAttributes(PushMessage message,
                                   Message.Builder iosBuilder) {
    ...
    private void processAttributes(PushMessage message,
                                   AndroidNotification.Builder buideer) {
        for (Map.Entry<String, Object> entry : message.getAttributes()
                                                      .entrySet()) {
            String key = entry.getKey();
            Object value = entry.getValue();
    ...
    private void doPush(PushMessage message,
                        PushPayload payload,
                        PushResponseReceiver responseReceiver) {
        if (payload == null) {
            return;
